...
class FormatConvertor:
    ...
    def read_input(self, annotation_file: str, text_file: str):
        ...
        #for idx, item in enumerate(input_annotations):
        #    print(idx, '\n', item)
    ...
    def parse_text(self):
        ...
        with open(self.output_file, 'w') as fo:
            for file_count, file_pair in enumerate(file_pair_list):
                ...
                #    pass
                while i < num_tokens:
                    #if text_tokens[i] == 'Sodium':
                        #break
                        #print(current_index)
                        #print(current_ann_start)
                    #print('current_index', current_index)
                    #print('current_ann_start', current_ann_start)
                    #print('text_tokens[i]', text_tokens[i])
                    if re.search(r'[^\w\s]', text_tokens[i]):
                    #if re.search(r'[.,!?;/:\-&%]', text_tokens[i]):
                        j = 0
                        ...
                        while j < token_len:
                            if current_index != current_ann_start:
                                fo.write(f'{tokens_puncs[j]} O\n')
                                #print('tokens_puncs[j]', tokens_puncs[j])
                                ...
                                while current_index < current_ann_end and j < token_len:
                                    #print('j', j)
                                    #print('current_index', current_index)
                                    #print('current_ann_end', current_ann_end)
                                    fo.write(f'{tokens_puncs[j]} {label}\n')
                                    current_index += len(tokens_puncs[j])
                                    #print('move', current_index)
                                    j += 1
                                    #print('move j', j)
                                annotation_count += 1
                        ...
                        #if current_index > 2615:
                        #    break
                        if current_index != current_ann_start:
                            #print('token', text_tokens[i])
